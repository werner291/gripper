...
use nphysics3d::object::{BodyPartHandle, DefaultBodyPartHandle, Body, BodyPart};
use nalgebra::{Point3, Isometry3};

use crate::physics::PhysicsWorld;
use std::option::Option;
use crate::gradient_descent_control;
...
impl Graphics {
    ...
    pub fn enable_trace(&mut self, target: DefaultBodyPartHandle, offset: Isometry3<f32>) -> TraceId {
    ...
    fn trace(&mut self, trace: &mut Trace, physics: &PhysicsWorld) {
            if self.frames_drawn % 10 == 0 {
                ...
                trace.points.push(target_part.position() * &trace.offset * Point3::new(0.0,0.0,0.0));

            }

            if trace.points.len() >= 2 {
                for i in 0..trace.points.len() - 1 {
                    self.window.draw_line(&trace.points[i], &trace.points[i + 1], &Point3::new(1.0, 0.0, 0.0));
                }
        }
    }
}
