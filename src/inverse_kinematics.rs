...
pub fn gradient_guided_planner(kinematic_model: &KinematicModel,
                               ...
                               attempts: usize,
                                ...
                                suggestion: Option<&[f32]>) -> Result<Vec<f32>,BestAttempt> {
    ...
    for attempt in 0..attempts {
        ...
        let mut angles : Vec<f32> = if let Some(suggestion) = suggestion {
            kinematic_model.links.iter().zip(suggestion.iter()).map(|(ln,sa)| {
                (sa + rng.gen_range(-PI..PI) * (attempt as f32/attempts as f32)).min(ln.max).max(ln.min)
            }).collect()
        } else {
            kinematic_model.links.iter().map(|ln| {
    ...
    if distance(&(&kinematic_model.origin * Point3::new(0.0, 0.0, 0.0)), target_position) > kinematic_model.chain_length() {
