...
use crate::robot::joint_map::{ArmJointMap, ArmJointVelocities, JointVelocities, FINGERS_OPEN};
...
pub fn joint_velocities_towards_angles(
    target_angles: &ArmJointMap<f32>,
    physics_world: &PhysicsWorld,
    robot: &RobotBodyPartIndex,
) -> ArmJointVelocities {
    let arm_joint_angles = ArmJointMap {
        swivel: get_joint::<RevoluteJoint<f32>>(physics_world, robot.swivel)
            .unwrap()
            .angle(),
        link1: get_joint::<RevoluteJoint<f32>>(physics_world, robot.link1)
            .unwrap()
            .angle(),
        link2: get_joint::<RevoluteJoint<f32>>(physics_world, robot.link2)
            .unwrap()
            .angle(),
        gripper: get_joint::<RevoluteJoint<f32>>(physics_world, robot.gripper)
            .unwrap()
            .angle(),
    };
