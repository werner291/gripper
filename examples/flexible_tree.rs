...
use nphysics3d::object::{Body, BodyPartHandle, ColliderDesc, DefaultBodyHandle, FEMVolume, FEMVolumeDesc, RigidBody, RigidBodyDesc, BodyStatus, DefaultColliderHandle, BodySet};

use gripper_experiment::{robot, spawn_utilities};
use gripper_experiment::robot::{ArmJointMap, JointMap, JointVelocities, RobotBodyPartIndex, GripperDirection};
...
use crate::ControllerState::{Grabbing, BringingToTarget, Idle};
...
fn main() {
    ...
    let (target_sn, target_bh, target_ch) = add_apple_collection_target(&mut p, &mut g, Point3::new(-2.0, 0.0, -2.0));
    let receptacle_position = p.bodies.rigid_body(target_bh).unwrap().position() * Point3::new(0.0, 0.0, 0.0);
...
impl ApplePickerController {
    fn new(apple_bh: DefaultBodyHandle, receptacle_position: Point3<f32>, physics_world: &PhysicsWorld, robot: &RobotBodyPartIndex) -> Self {
    ...
    fn make_motion_plan(&self, robot: &RobotBodyPartIndex, p: &PhysicsWorld, target_position: &Point3<f32>) -> ArmJointMap<f32> {

        let joint_angles: [f32; 4] = robot::arm_joint_angles(&p, &robot).into();

        let plan_result = inverse_kinematics::gradient_guided_planner(&self.kinematic_model,
                                                                      target_position,
...
fn add_apple_collection_target(p: &mut PhysicsWorld, g: &mut Graphics, middle_point: Point3<f32>) -> (SceneNode, DefaultBodyHandle, DefaultColliderHandle) {
...
fn spawn_apple_on_branch(mut p: &mut PhysicsWorld, mut g: &mut Graphics, branch_bh: DefaultBodyHandle, target_attachment_point: &Point3<f32>) -> DefaultBodyHandle {
    ...
    let (mut apple_sn, apple_bh) = spawn_utilities::spawn_ball(&mut p, &mut g, 0.35, apple_position);
...
impl ApplePickerController {
    fn apply_state_transitions(&mut self, physics_world: &PhysicsWorld, robot: &RobotBodyPartIndex) {

        let joint_angles : [f32; 4] = robot::arm_joint_angles(&physics_world, &robot).into();
        let tip_position = self.kinematic_model.predict(&joint_angles).tip_position * Point3::new(0.0, 0.0, 0.0);
        ...
        match self.state {
            ...
            ControllerState::Approaching { .. } => {
                let apple_position = physics_world.bodies.rigid_body(self.apple_bh).expect("Apple should be in the physics world when approaching.").position() * Point3::new(0.0, 0.0, 0.0);
            ...
            ControllerState::Grabbing => {
                if grabbed(physics_world, robot, self.apple_bh) {
                    self.state = BringingToTarget {
                        motion_plan: self.make_motion_plan(robot, physics_world, &self.receptacle_position)
                    };
                }
            }
...
impl ApplePickerController {
    fn compute_joint_velocities(&mut self, robot: &RobotBodyPartIndex, physics_world: &PhysicsWorld) -> JointMap<f32> {
        ...
        match self.state {
            ...
            ControllerState::Approaching { motion_plan } => {
                let arm_target_velocities = to_angles::joint_velocities_towards_angles(&motion_plan, &physics_world, &robot);

                JointVelocities::from_arm_and_finger(arm_target_velocities, robot::FINGERS_OPEN)
            }
            Grabbing => {
                JointVelocities::from_arm_and_finger(ArmJointMap {
                    swivel: 0.0,
                    link1: 0.0,
                    link2: 0.0,
                    gripper: 0.0
            ...
            ControllerState::BringingToTarget { motion_plan } => {
                ...
                JointVelocities::from_arm_and_finger(arm_target_velocities, robot::FINGERS_CLOSE)
            }
        }
    }
}
